...
class Slashings extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: {},
  ...
  static async getInitialProps() {
    const data = await action
      .getData("validators/slashings?page=1&perPage=10")
      .then((res) => res);
  ...
  handlePageClick = (data) => {
    ...
    this.setState({ page: page, loader: true }, () => {
      action
        .getData(
          `validators/slashings?page=${page}&perPage=${this.state.perPage}`
        )
        .then((res) => {
  ...
  handleChange = (event) => {
    this.setState(
      ...
      () => {
        action
          .getData(
            `validators/slashings?page=${this.state.page}&perPage=${this.state.perPage}`
          )
          .then((res) => {
            if (res && res.message === "Sucess") {
              this.setState({ mainData: res, loader: false });
  ...
  handleSelect = (key) => {
    ...
    if (key === "1") {
      action.getData("validators/slashings?page=1&perPage=10").then((res) => {
        this.setState({
          mainData: res,
          loader: false,
          showData: true,
  ...
  render() {
    ...
    return (
      <Layout
        websiteTitle="Slashed Validators - Eth | Cryptocurrency"
      ...
      >
        <Container>
          <section>
            <InnerPageBanner pageTitle="Slashed Validators" />
            <Row>
            <Col md={12}>
...
export default withRouter(Slashings);
